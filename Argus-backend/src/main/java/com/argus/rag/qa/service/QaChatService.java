...
import static com.argus.rag.qa.constant.QaConstant.*;
...
public class QaChatService {
    ...
    private LlmCallResult getStructuredAnswer(
        ...
        try {
            LlmRawResult raw = callLlm(groupId, userPrompt, evidenceBundle);
            if (raw.text() == null) {
                return new LlmCallResult(null, raw.usage());
            }
            KnowledgeAnswerOutput output = objectMapper.readValue(raw.text(), KnowledgeAnswerOutput.class);
            return new LlmCallResult(output, raw.usage());
    ...
    /**
     * 回退解析：当结构化输出失败时，再次调用大模型并用 {@link QaAnswerParser} 解析。
     */
    private LlmCallResult parseFallbackAnswer(
        ...
        try {
            ...
            log.info(
                    ...
                    groupId,
                    evidenceBundle.documents().size(),
                    raw.text() == null ? 0 : raw.text().length());
            KnowledgeAnswerOutput output = answerParser.parse(raw.text());
            return new LlmCallResult(output, raw.usage());
     ...
     * 调用大模型并返回原始文本和用量信息。
     */
    private LlmRawResult callLlm(Long groupId, Prompt userPrompt, RetrievedEvidenceBundle evidenceBundle) {
        ...
        if (chatResponse != null) {
            ...
            usageInfo = extractUsageInfo(chatResponse.getMetadata().getUsage(), false);
        }
        return new LlmRawResult(text, usageInfo);
    }
     ...
     * 内部 LLM 原始调用结果。
     */
    private record LlmRawResult(String text, UsageInfo usage) {
    }
