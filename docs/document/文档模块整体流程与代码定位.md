...
### 11.5 当前 document 主链路统一采用 Spring 事件 + @Async + @Retryable
...
也就是说，当前你最应该掌握的是这条事件驱动链路，而不是把它讲成“任务表 Worker 调度系统”。当前仓库也已经不再保留 `ingestion_jobs` 相关表和 mapper。
...
2. 不要把摄入链路讲成复杂任务调度平台，当前更真实的说法是“主流程是 Spring 事件异步摄入”。
