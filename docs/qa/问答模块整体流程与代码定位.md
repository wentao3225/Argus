...
混合检索层（HybridChunkRetrievalService / QueryPlanningService）
    ↓
检索基础设施层（PgVectorRetrievalAdapter / ElasticsearchChunkIndexService / DocumentChunkMapper）
    ↓
...
- Elasticsearch 中的 `dd_rag_document_chunks` 索引
...
### 第 18 步：关键词检索通过 ElasticsearchChunkIndexService 执行
...
进行过滤，并对 `fileName + chunkText` 做匹配和 rescore。

### 第 19 步：关键词检索失败时会降级为空结果，而不是直接打断问答

位置：

- `ElasticsearchChunkIndexService.search()`

这意味着当前混合检索对 ES 故障有一定容错能力：
...
8. `Argus-backend/src/main/java/com/argus/rag/engine/elasticsearch/ElasticsearchChunkIndexService.java`
