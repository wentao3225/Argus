...
### 5.5 `DocumentParserFactory` 当前采用“简单工厂 + 策略模式”选择 parser
...
- 四个具体 parser 都是 Spring Bean
- `DocumentParserFactory` 通过注入的 `List<DocumentParser>` 按扩展名调用 `supports()`，返回第一个命中的策略实现

所以从当前主路径看：

- 新增 parser 不需要回工厂里手工注册
- 只需要新增一个实现 `DocumentParser` 的 Spring Bean，并正确实现 `supports()`
- 当前没有单独维护 `extension -> parser` 注册表，而是运行时按策略接口做简单选择
...
### 13.2 `DocumentParserFactory` 当前是简单工厂，parser 本身是具体策略实现

位置：

- `DocumentParserFactory.getParser()`

所以扩展新文件类型时，通常不需要改工厂本身；更关键的是新增 parser Bean 并保证 `supports()` 判断清晰、唯一。
...
### 13.7 当前仓库已经不再保留 ingestion_jobs Worker 调度定义

从当前主控路径看，reader/parser/transformer/chunk/vector 这条技术链统一建立在 Spring 事件驱动的异步摄入之上，不再存在独立的 ingestion_jobs Worker 调度分支。
