...
- `Argus-backend/src/main/java/com/argus/rag/common/enums/IngestionJobStatus.java`
- `Argus-backend/src/main/java/com/argus/rag/common/enums/IngestionJobType.java`
...
- `Argus-backend/src/main/resources/mappers/ingestion/IngestionJobMapper.xml`
...
- `sql/schema.sql` 中的 `ingestion_jobs`
...
### 5.4 ingestion_jobs 表

`ingestion_jobs` 表和对应的 `IngestionJobMapper`、`IngestionJobEntity` 已经存在，字段包括：
...
但从当前主链路看，它更像“预留出来的任务表调度能力”，不是现在 document 上传完成后真正走的第一条执行路径。
...
任务表枚举 `IngestionJobStatus` 虽然已经定义了：
...
但当前主链路不是先创建 job 再驱动执行，而是事件驱动异步执行后直接更新 `documents`。
...
### 11.1 当前主链路是 Spring 事件驱动，不是 ingestion_jobs Worker 调度
...
从当前调用关系看，`IngestionJobMapper` 只有定义，没有看到实际业务类在调用它。
...
### 11.3 ParserFactory 当前是手工 new 解析器，不是自动发现式注册
...
所以如果后面新增 CSV、HTML 等解析器，当前设计下还需要手动把它加进工厂，而不是只加一个 `@Component` 就自动生效。
...
1. 不要把当前实现讲成“完整的分布式 Worker 调度系统”，因为 ingestion_jobs 目前更像预留能力。
